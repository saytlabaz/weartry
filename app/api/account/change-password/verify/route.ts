...
export async function POST(req: NextRequest) {
  ...
  const { data: pending, error: pendingError } = await supabaseNextAuth
    ...
    .eq("change_type", "password_change")
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle();

  if (pendingError) {
    console.error("[change-password/verify] pending lookup error:", pendingError);
  ...
  if (updateError) {
    console.error("[change-password/verify] user update error:", updateError);
    return NextResponse.json({ error: "server_error" }, { status: 500 });
  }
